...
def register_user(app):
    ...
    app.register_user(name, password)
...
class LanguageLearningApp:
    ...
    def register_user(self, name, password):
        this_user = users_collection.find_one({"name": name})
        if this_user:
    ...
    def display_words(self):
        print("Words")
        for i, words in enumerate(self.word_list[:100], start=1):
            print(f"{i}. {words['word']} - {words['meaning']}")
